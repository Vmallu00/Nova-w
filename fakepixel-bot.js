...
class FakePixelBot extends EventEmitter {
  constructor(config = {}) {
    ...
    this.lastSpawnDistanceLog = 0;

    this.lastLoggedServerMessage = "";

    this.lastLoggedServerMessageTime = 0;
  ...
  attachInternalListeners() {
    ...
    this.on(
      ...
      () => {
        this.log(
          "Authentication timeout. Continuing with FakePixel flow."
        );
        ...
        /*
         * IMPORTANT:
         * Do NOT wait for an exact spawn/hub
         * coordinate.
         *
  ...
  handleServerMessage(text) {
    ...
    if (
      ...
      this.log(
        "Registration confirmation detected."
      );
   ...
   * Compatibility only.
   *
   * The bot no longer needs to reach
   * these coordinates before /visit.
   * ================================================== */
  ...
  async handleMainSpawn() {
    ...
    this.log(
      "Skipping coordinate detection. Direct /visit flow enabled."
    );
  ...
  async handleGameMenu() {
    /*
     * This is retained in case the server
     * actually requires Game Menu.
     *
     * Normally the new flow skips it.
     */
  ...
  async handleVisiting() {
    ...
    /*
     * Prevent duplicate /visit commands.
     */
    if (
      this.targetVisitAttempted
    ) {
    ...
    try {
       ...
       * Close any old GUI.
       *
       * This prevents an old Game Menu
       * from blocking /visit.
       */
      ...
      if (
        ...
        try {
          this.log(
            "Closing existing GUI before /visit."
          );
      ...
      if (
        ...
        /*
         * Some FakePixel versions may
         * process /visit without opening
         * the expected GUI.
         *
        ...
        this.log(
          "Visit GUI did not open. Waiting for teleport/visit response."
        );
      ...
      if (item) {
        ...
        this.log(
          "Visit slot 12 is empty."
        );
      }

      this.log(
        "Trying RIGHT CLICK on visible slot 12."
      );
      ...
      try {
        ...
        this.log(
          "Right-click on Visit slot 12 sent."
        );
      } catch (error) {
        this.log(
          `Right-click failed: ${error.message}`
        );
      }
      ...
      if (
        ...
        this.log(
          "Visit GUI is still open. Trying LEFT CLICK on slot 12."
        );
        ...
        try {
          ...
          this.log(
            "Left-click fallback sent."
          );
        } catch (error) {
          this.log(
            `Left-click fallback failed: ${error.message}`
          );
        }
      }

      /*
       * Give FakePixel time to teleport.
       */
      ...
      this.log(
        `Visit flow error: ${error.message}`
      );

      /*
       * Do not immediately spam /visit.
       *
       * Let the bot continue toward
       * target-island and AFK.
       */
  ...
  async handleTargetIsland() {
     ...
     * island coordinate.
     *
     * FakePixel target islands can have
    ...
    this.log(
      `Target visit flow completed. Current position: ${this.positionText()}`
    );
  ...
  startAfk() {
    ...
    this.log(
      "Starting AFK movement and jumping."
    );

    this.log(
      `AFK settings: move=${this.moveInterval}ms, jump=${this.jumpInterval}ms, distance=${this.moveDistance}`
    );
  ...
  checkLocation() {
    ...
    /*
     * Location is now informational.
     *
     * It does NOT control whether /visit
     * happens.
     */
    ...
    if (
      ...
      this.log(
        `Main spawn detected: ${mainDistance.toFixed(2)} blocks away.`
      );
    ...
    if (
      ...
      this.log(
        `SkyBlock Hub detected: ${hubDistance.toFixed(2)} blocks away.`
      );
  ...
  shouldLogServerMessage(text) {
    ...
    /*
     * Suppress repeated identical
     * server messages within 5 sec.
     */
    ...
    if (
      clean ===
        this.lastLoggedServerMessage &&
      now -
        this.lastLoggedServerMessageTime <
        5000
    ) {
      return false;
    }
