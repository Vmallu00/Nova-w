...
const MAIN_SPAWN = new Vec3(
  Number(
    process.env.MAIN_SPAWN_X || -52.5
  ),
  Number(
    process.env.MAIN_SPAWN_Y || 95.74244
  ),
  Number(
    process.env.MAIN_SPAWN_Z || 0.5
  )
);

const SKYBLOCK_HUB = new Vec3(
  Number(
    process.env.SKYBLOCK_HUB_X || -2.5
  ),
  Number(
    process.env.SKYBLOCK_HUB_Y || 70.0625
  ),
  Number(
    process.env.SKYBLOCK_HUB_Z || -68
  )
);
...
class FakePixelBot extends EventEmitter {
  ...
  attachInternalListeners() {
    ...
    this.on(
      ...
      () => {
        this.log(
          "Authentication timeout. Continuing to FakePixel spawn detection."
        );
  ...
  attachMinecraftListeners(bot) {
    bot.once(
      ...
      () => {
        ...
        this.log(
          `Minecraft spawned. ${this.username} is online.`
        );
  ...
  handleServerMessage(text) {
    ...
    if (
      ...
      this.log(
        "FakePixel welcome message detected."
      );
  ...
  async handleMainSpawn() {
    ...
    if (
      ...
      this.log(
        `Main FakePixel spawn detected (${distance.toFixed(2)} blocks).`
      );
  ...
  async handleGameMenu() {
    ...
    try {
      ...
      if (
        ...
        this.log(
          "Game Menu interaction was unsuccessful. Retrying."
        );
  ...
  async openGameMenuAndClickHub() {
    ...
    try {
      /*
       * Game Menu compass is slot 1
       * from the user's FakePixel setup.
       *
       * Mineflayer uses zero-based slots.
       */
      ...
      /*
       * Right-click / use the compass.
       */
      this.bot.activateItem();

      /*
       * Give FakePixel time to
       * send/open the menu.
       */
      ...
      this.log(
        "SkyBlock Hub menu slot 21 clicked."
      );
  ...
  async handleTargetIsland() {
    ...
    /*
     * FakePixel island coordinates
     * are dynamic, so do not require
     * a fixed XYZ here.
     *
     * First allow the teleport to finish.
     */
    if (
      elapsed <
      2500
    ...
    this.log(
      `Target island arrival assumed. Current position: ${this.positionText()}`
    );
  ...
  startDurationTimer() {
    ...
    if (
      ...
      this.log(
        "No valid duration configured. Bot will remain online."
      );
    ...
    this.log(
      `AFK duration set to ${hours} hour(s).`
    );
    ...
    this.durationTimer =
      setTimeout(() => {
        ...
        this.stop()
          .catch(error => {
            this.log(
              `Duration stop error: ${error.message}`
            );
          });
      }, milliseconds);
  }
